...
The application of [[concepts/long-running-agentic-coding]] to scientific-computing tasks: reimplementing numerical solvers, porting legacy scientific software, building differentiable versions of existing models, debugging large research codebases against reference implementations. The thesis — advanced in the April 2026 Anthropic post by [[entities/siddharth-mishra-sharma]] — is that this class of task **fits the autonomous-agent posture unusually well** because it tends to satisfy all three of the enabling conditions simultaneously: well-scoped deliverables, quantifiable success criteria, and tolerance for occasional rather than continuous human oversight (Source: [[sources/2026-04-04-anthropic-long-running-claude-scientific-computing]]).
...
1. **Reference implementations often exist.** Most mature scientific domains have a canonical codebase (or two) that the community already trusts. These are natural [[concepts/test-oracle-for-agents]]: [[entities/class-boltzmann-solver]] for CMB cosmology, LAMMPS for molecular dynamics, CASTEP or VASP for DFT electronic structure, NCBI BLAST for sequence alignment. An agent building a reimplementation in a new substrate has an immediate ground truth.
...
- **Progress is clunky but sustained.** Despite the above, the accuracy trajectory is monotonic toward the target. The combination of persistent memory ([[concepts/agent-persistent-memory]]), the test oracle, and occasional human steering keeps the project moving even when individual steps are inefficient.
...
- **The parallelism frontier is uneven.** Some scientific codebases decompose cleanly (parsers, utility libraries, per-test parallelization); others — like deeply coupled numerical pipelines — do not. The [[entities/anthropic-c-compiler-project]] succeeded with a swarm of ~2,000 parallel sessions; CLAX succeeded with a single sequential agent. **The shape of the codebase determines the topology of the agent deployment.** See [[entities/anthropic-c-compiler-project]] for the contrast.

## Relationship to other wiki themes

- **Extension of [[concepts/long-running-agentic-coding]] into a specific domain.** This page is the domain-specific case; the general pattern lives at [[concepts/long-running-agentic-coding]].
- **Reinforces [[analyses/own-your-substrate]].** The researcher owns the codebase, the HPC allocation, the git repo, and the accumulated domain understanding. Nothing about the workflow rents compounding infrastructure.
- **Adjacent to but distinct from [[concepts/llm-driven-algorithm-discovery]].** AlphaEvolve generates and evolves new algorithms; agent-driven scientific computing typically reimplements existing ones. Both use LLMs as producers of code with an evaluation loop, but the novelty target differs — AlphaEvolve searches for beats-state-of-the-art; CLAX searches for matches-state-of-the-art-in-a-new-substrate. The latter is a much easier problem structurally.

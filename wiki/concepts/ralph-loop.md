...
1. **The success criterion** — expressed as a precise, measurable condition tied to the [[concepts/test-oracle-for-agents]]. "0.1% accuracy across the entire parameter range" is the example; "all tests pass and coverage is above 90%" is another.
...
So the right mental model is: Ralph is a workaround for [[concepts/agentic-laziness]] at current capability, not a permanent architectural component. The loop will probably become unnecessary once models stop declaring completion prematurely.
...
- **Filter loop on the producer.** Ralph is a thin instance of the general pattern analysed in [[concepts/test-oracle-for-agents]]: the LLM produces artifacts, and a filter/oracle decides whether the producer is done. In Ralph's case, the filter is the success criterion check; in Karpathy's [[concepts/llm-knowledge-bases]], it's the lint pass; in AlphaEvolve ([[concepts/llm-driven-algorithm-discovery]]) it's the exploitability metric. Same architecture, different filter.
- **Dependency on [[concepts/test-oracle-for-agents]].** Ralph only works if there's a trustworthy way to tell whether the success criterion has actually been met. Without a real oracle, Ralph just loops forever on an agent that keeps claiming "done" falsely. Ralph is the control mechanism *on top of* a valid oracle, not a substitute for one.
